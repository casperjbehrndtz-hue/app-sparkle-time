...
// ── Article fetcher factory ──

interface ArticleFetcherOptions {
  table: string;
  select: string;
  siteName: string;
  /** Full site URL for mainEntityOfPage, e.g. "https://www.parfinans.dk" */
  siteUrl?: string;
  /** URL prefix for breadcrumbs, e.g. "/guides" */
  urlPrefix: string;
  /** Label for the parent breadcrumb, e.g. "Guides" */
  parentLabel: string;
  slugColumn?: string;
  statusValue?: string;
  fields?: {
...
export function createArticleFetcher(opts: ArticleFetcherOptions) {
  const {
    table,
    select,
    siteName,
    siteUrl: articleSiteUrl,
    urlPrefix,
    parentLabel,
    slugColumn = "slug",
  ...
  return async function fetchArticle(
    ...
    try {
      ...
      if (!res.ok) {
        console.warn(`[dk-seo] ${table} fetch failed: ${res.status} for slug "${slug}"`);
        return null;
      }
      ...
      const keyword = row[keywordField] || undefined;
      ...
      console.warn(`[dk-seo] ${table} fetch error for slug "${slug}":`, (err as Error).message);
      return null;
    }
  };
}
