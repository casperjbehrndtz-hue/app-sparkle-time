    ...
    try {
      ...
      if (truncated && jsonStart !== -1) {
        ...
        try {
          ...
          console.error("bank-statement-ocr: JSON repair failed. Raw text:", text.slice(0, 500));
        }
      }
      if (!parsed) {
        console.error("bank-statement-ocr: JSON parse failed. Raw text:", text.slice(0, 500));
        return new Response(
