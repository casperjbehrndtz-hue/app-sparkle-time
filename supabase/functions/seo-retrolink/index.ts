...
function findInsertionPoint(
  ...
  // Strategy 2: Add to "Laes ogsaa" section
...
Deno.serve(async (req) => {
  ...
  if (action === "status") {
    ...
    const { data: clusters } = await supabase
      .from(CLUSTER_TABLE)
      .select("pillar_slug, cluster_slug");
    ...
    for (const c of clusters || []) {
      const { count } = await supabase
        .from(LINKS_TABLE)
        .select("*", { count: "exact", head: true })
        .eq("source_slug", c.cluster_slug)
        .eq("target_slug", c.pillar_slug);
    ...
    return json({
      ...
      cluster_coverage_pct: (clusters || []).length > 0
        ? Math.round((clusterLinked / (clusters || []).length) * 100)
        : 0,
    });
  }
  ...
  if (action === "auto_link") {
    ...
    if (artErr || !articles) {
      return json(
        { error: "Failed to fetch articles", detail: artErr?.message },
        500,
      );
    }

    // 2. Find link-starved articles
    const starved: typeof articles = [];
    for (const art of articles) {
      const { count } = await supabase
        .from(LINKS_TABLE)
        .select("*", { count: "exact", head: true })
        .eq("target_slug", art.slug);
      if ((count || 0) < MIN_INCOMING_LINKS) {
        starved.push(art);
      }
    }
      ...
      try {
        ...
        for (const c of clusters || []) {
          if (c.cluster_slug === target.slug && c.pillar_slug) {
            relatedFromClusters.push({
              slug: c.pillar_slug,
              anchor: c.anchor_text || target.title,
              type: "cluster",
            });
          }
          if (c.pillar_slug === target.slug && c.cluster_slug) {
            relatedFromClusters.push({
              slug: c.cluster_slug,
              anchor: c.reverse_anchor_text || target.title,
              type: "cluster",
            });
          }
        }

        // b) Find semantically related by keyword overlap
        ...
        for (const source of sources) {
          ...
          console.log(
            `Linked: ${source.slug} -> ${target.slug} (${source.linkType})`,
          );
          linked.push({
            source: source.slug,
            target: target.slug,
    ...
    // 5. Update cluster links (ensure bidirectional)
    for (const c of clusters || []) {
      try {
        ...
        if (
          ...
          if (point) {
            ...
            const { count: ex } = await supabase
              ...
              .eq("target_slug", c.pillar_slug);
            if ((ex || 0) === 0) {
              await supabase.from(LINKS_TABLE).insert({
                source_slug: c.cluster_slug,
                target_slug: c.pillar_slug,
                anchor_text: c.anchor_text || pillarArt?.title ||
                  c.pillar_slug,
                link_type: "cluster",
                auto_injected: true,
              });
            ...
            console.log(
              `Cluster link: ${c.cluster_slug} -> ${c.pillar_slug}`,
            );
            linked.push({
              source: c.cluster_slug,
              target: c.pillar_slug,
        ...
        if (
          ...
          if (point) {
            ...
            const { count: ex } = await supabase
              .from(LINKS_TABLE)
              .select("*", { count: "exact", head: true })
              .eq("source_slug", c.pillar_slug)
              .eq("target_slug", c.cluster_slug);
            if ((ex || 0) === 0) {
              await supabase.from(LINKS_TABLE).insert({
                source_slug: c.pillar_slug,
                target_slug: c.cluster_slug,
                anchor_text: c.reverse_anchor_text || clusterArt?.title ||
                  c.cluster_slug,
                link_type: "cluster",
                auto_injected: true,
              });
            ...
            console.log(
              `Cluster link: ${c.pillar_slug} -> ${c.cluster_slug}`,
            );
            linked.push({
              source: c.pillar_slug,
              target: c.cluster_slug,
              anchor: c.reverse_anchor_text || "",
              type: "cluster",
            });
          }
        }
